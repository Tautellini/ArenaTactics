...
class HomeViewModel(
    ...
    // Called when addon is tapped. Transitions gameModeRow: Idle/Ready → Loading → Ready/Error.
    // Guard: if gameModeRow is already Ready AND lastLoadedAddonId == addonId, do nothing (avoids spinner flash on re-tap after back-navigation).
    // For a different addonId, always reloads (no guard).
    // Must use catch(Throwable) in the coroutine body; on Throwable emits GameModeRowState.Error.
    fun loadGameModes(addonId: String)

    // Called when addon is deselected. Resets gameModeRow to Idle and clears lastLoadedAddonId.
    fun resetGameModes()

    // Internal state (not exposed):
    // private var lastLoadedAddonId: String? = null
    // Set to addonId when Ready is emitted; cleared in resetGameModes()
}
...
3. **Tapping a different addon while section or bracket rows are visible** → call `viewModel.resetGameModes()` first (sets gameModeRow to Idle immediately); then set `selection = HomeSelection(addon = newAddon)`; then call `viewModel.loadGameModes(newAddon.id)`. Section row stays visible (`selection.addon != null`); bracket row collapses (`selection.section == null`); Tactics tile shows spinner during load — no stale game modes visible
...
- **`ScreenNavigationTest.kt`** (`commonTest/kotlin/.../navigation/ScreenNavigationTest.kt`) must be updated — it references `Screen.AddonHub` and `Screen.GameModeSelection` which will no longer exist. Required changes:
  ...
  - `buildStackMatchupDetail`: update to expect size 4; assert `stack[0]=AddonSelection`, `stack[1]=CompositionSelection("tbc_anniversary","tbc_anniversary_2v2")`, `stack[2]=MatchupList(...)`, `stack[3]=MatchupDetail(...)`
...
- **Project rule: "do not place all Models in a singular file"**: `HomeState` and `GameModeRowState` are ViewModel state types — they describe the ViewModel's output shape, not the domain data layer. They belong in `HomeViewModel.kt` alongside the ViewModel that produces them. This is consistent with the intent of the project rule (which targets domain model files like `Composition.kt`, `Spec.kt`) and with how other ViewModels in the codebase co-locate their state types.
